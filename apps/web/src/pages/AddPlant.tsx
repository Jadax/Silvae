...
import { addDays, POT_TYPES, SOIL_TYPES } from "@silvae/core";
...
export default function AddPlant() {
  ...
  function next() {
    ...
    if (step === 2 && !species) {
      setError("Pick a plant type so we can build its care plan.");
      return;
    }
  ...
  async function submit() {
    ...
    await addPlant({
      ...
      locationType,
      potType,
      soilType: isGlassPot ? "well-draining" : soilType,
  ...
  return (
    <div className="setup-page">
      <div className="page-intro">
        <span className="eyebrow">Add a plant</span>
        <h1>New plant</h1>
        <p>Name it, pick its type, and we'll set up a care plan.</p>
      </div>
      <ol className="stepper" aria-label={`Step ${step} of 3`}>
        {["Name it", "Find its type", "Set its home"].map((label, index) => (
      ...
      <section className="card setup-card">
        {step === 1 && <>
          ...
          <p className="muted">A photo helps, but you can skip it and add one later.</p>
          <div className="photo-picker">
            {photo ? <img src={photo} alt="Your plant" /> : <span aria-hidden>\uD83C\uDF3F</span>}
            <div className="photo-actions">
              <label className="photo-button">
                {photo ? "Change photo" : photoBusy ? "Preparing..." : "\uD83D\uDCF7 Take photo"}
          ...
          <label>
            What do you call it?
            <input value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Momo, Green giant" autoFocus />
          </label>
        </>}

        {step === 2 && <>
          <h2>What kind of plant is {name || "this"}?</h2>
          <p className="muted">Search by common or scientific name, or try a photo.</p>
          <div className="identify-card">
            <div className="identify-row">
              <span aria-hidden>{'\uD83D\uDCF7'}</span>
              <div>
                <strong>Not sure what it is?</strong>
                <p className="muted">Take a photo and we'll try to guess. You confirm before anything is set.</p>
              </div>
            </div>
            <div className="photo-actions">
              <label className="btn secondary photo-button identify-button">
                {idState === "busy" ? "Looking..." : "\uD83D\uDCF7 Take photo"}
              ...
              <label className="btn secondary photo-button identify-button">
                {"\uD83D\uDCC1 Choose from album"}
                <input type="file" accept="image/*" disabled={idState === "busy"} onChange={(event) => { const f = event.target.files?.[0]; if (f) void identifyFromPhoto(f); event.currentTarget.value = ""; }} />
              </label>
            </div>
            {idState === "done" && idMatches.length > 0 && (() => {
              ...
              return (
                <p className={`identify-result ${best?.inCatalog ? "" : "warn"}`}>
                  {best?.inCatalog
                    ? <>Looks like <strong>{best.commonNames[0] ?? best.scientificName}</strong>{best.confidence !== undefined ? ` (${Math.round(best.confidence * 100)}%)` : ""}. Confirm below or pick another.</>
          ...
          {matches.length === 0 && <p className="muted">No matches yet. Try a simpler name.</p>}
        </>}

        {step === 3 && <>
          <h2>Where will {name || "your plant"} live?</h2>
          <p className="muted">Outdoor plants get real weather data: frost and heat warnings, rain-aware scheduling.</p>
          ...
          <div className="form-grid">
            ...
            {isGlassPot && <p className="muted" style={{ gridColumn: "span 1", alignSelf: "center" }}>Glass pots typically use water only.</p>}
            <label>Pot size (cm)<input type="number" min="1" value={potSizeCm} onChange={(event) => setPotSizeCm(event.target.value)} /></label>
          </div>
          <label>Notes <span className="optional">Optional</span><textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="A gift from Grandma, by the sunny window..." rows={3} /></label>
        </>}
